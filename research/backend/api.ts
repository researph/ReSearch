...
app.post("/api/generateLetter", async (req: Request, res: Response) => {
  try {
    ...
    if (!response.ok || !data || !data.candidates || !data.candidates[0].content) {
      console.error("❌ Gemini API Error:", data);
      res.status(500).json({ error: "Gemini API failed to generate a response." });
      return;
    }

    console.log("Gemini API Response:", data);

    res.status(200).json({ letter: data.candidates[0].content });
  } catch (error) {
